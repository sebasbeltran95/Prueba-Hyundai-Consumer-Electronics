...
class Estados extends Component
{
    ...
    {
        if($this->search == ""){
            return ModelsEstados::orderBy('id','DESC')->paginate(5);
        } else {
        ...
        public function crear()
    {
        try { 
            ...
            $user = new ModelsEstados();
            $user->nombre =  $this->nombre;
            $user->save();

            $this->reset();
            $msj = ['!Registrado!', 'Se registro el Estado', 'success'];
            $this->emit('ok', $msj);
        ...
        public function actua()
    {
        try { 
            ...
            $data = ModelsEstados::find($this->idx);
            $data->nombre = $this->nombrex;
    
            $data->save();
            $msj = ['!Actualizado!', 'Se actualizo el Estado', 'success'];
            $this->emit('ok', $msj);
    ...
    {
        return view('livewire.estados')->extends('layouts.plantilla_back')->section('contenido');
    }
}
